...
<h1 style="text-align:center;">VERIFIKASI PKL</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                ...
                <tr>
                    ...
                    <td class="px-4 py-4">{{ $item->nilaipkl }}</td>
                    <td>
                        @if ($item->isverified == 0)
                        <form action="/dashboarddosen/verifikasipkl/verify/{{$item->id}}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-success">Setujui</button>
                        </form>
                        @else
                        <form action="/dashboarddosen/verifikasipkl/unverify/{{$item->id}}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-danger">Batal</button>
                        </form>
                        @endif
                    </td>
                    <td><a href="/dashboarddosen/verifikasipkl/download/{{ $item->id }}"
